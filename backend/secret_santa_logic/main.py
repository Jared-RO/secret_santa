...
from secret_santa_logic.schemas import SecretSantaRequest, AsignacionResponse
...
async def procesar_secret_santa(request: SecretSantaRequest, background_tasks: BackgroundTasks):
    participantes = request.participantes
    
    if len(participantes) < 2:
        raise HTTPException(status_code=400, detail="Deben haber al menos 2 participantes.")
        
    # Validar duplicados
    correos = [p.correo.lower() for p in participantes]
    if len(correos) != len(set(correos)):
        raise HTTPException(status_code=400, detail="No se permiten correos duplicados.")
        
    nombres = [p.nombre.strip() for p in participantes]
    if len(nombres) != len(set(nombres)):
        raise HTTPException(status_code=400, detail="No se permiten nombres duplicados.")
